...
async function start() {
  try {
    ...
    await page.goto("https://bscscan.com/token/0x0e09fabb73bd3ade0a17ecc321fd13a19e81ce82?a=0x86ac3974e2bd0d60825230fa6f355ff11409df5c", {
    ...
    if (Number(finalBalance) > 10 && Number(finalBalance) !== cakePrev) { //send notification if balance over 10 cake and notification not already sent
      cakePrev = Number(finalBalance)
      push.send("CAKE on Venus", `CAKE Liquidity: ${Number(finalBalance)} CAKE`); //sends notification to phone
    }
    ...
    await axios.get(URL).then(res => {
      for (let i in res.data.data.markets) {
        if (res.data.data.markets[i].address === '0x86ac3974e2bd0d60825230fa6f355ff11409df5c') {
          if (Number(res.data.data.markets[i].liquidity) > 150) { //notify if over 150 usd 
            push.send("CAKE on Venus", `CAKE Liquidity: $ ${Number(res.data.data.markets[i].liquidity).toFixed(2)}`);
...
var runBot = schedule.scheduleJob("* * * * *", async function () { //runs every minute 
